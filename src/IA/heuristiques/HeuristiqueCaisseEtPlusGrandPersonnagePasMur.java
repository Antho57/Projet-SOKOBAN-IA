 ...
 *
 */
public class HeuristiqueCaisseEtPlusGrandPersonnagePasMur implements Heuristique{
	...
	public int calculerHeuristique(Etat etat, List<int[]> emplacements) {
		...
		// ########################################
		// 1. calcule la distance entre le personnage et la caisse la plus loin non placee
		...
		for (Caisse c : liste) {
			...
			if (!c.estBienPlace()) {
				...
				if (distanceCaissePerso > distanceMax)
					distanceMax = distanceCaissePerso;
			}
		}

		// heuristique = distance
		// (distance pour être a coté + 1 car caisse mal placee = distanceMin)
		heuristique = distanceMax;
