...
/*
 * Class qui repérsente le labyrinthe de jeu, qui contient les caisses et le
 * personnage
 */
public class Labyrinthe implements Sujet {
	...
	public Labyrinthe(int niv) {
		...
		try {
			...
			InputStreamReader stream = new InputStreamReader(
					new FileInputStream("./Ressources/Niveaux/sokoban" + niv + ".xsb"));
		...
		this.ia = new IA(this, new HeuristiqueCaisseEtPersonnage());
	}
	...
	/*
	 * Méthode qui permet de savoir si la case passé en parametre est occupé par une
	 * caisse ou par le personnage
	 * 
				...
				if (!this.isOccupe(caseSuivante2) && !this.isMur(caseSuivante2)) {
					...
					for (int i = 0; i < 9; i++) {
						for (int j = 0; j <= 7; j++) {
							...
							case "Sol":
								if (place.getOccupantCaisse() != null)
									rep[j][i] = new Sol(j, i, new Caisse(j, i, false), true);
								else
									rep[j][i] = new Sol(j, i, null, false);
								break;
							case "Emplacement":
								if (place.getOccupantCaisse() != null)
									rep[j][i] = new Emplacement(j, i, new Caisse(j, i, false), true);
								else
									rep[j][i] = new Emplacement(j, i, null, false);
								break;
							case "Mur":
								rep[j][i] = new Mur(j, i);
								break;
					...
					if (grille[caseSuivante2.getX()][caseSuivante2.getY()].getClass().getSimpleName()
							.equals("Emplacement")) {
						c.setBienPlace(true);
					} else {
